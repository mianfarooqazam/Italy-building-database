...
const SheetCalculation = ({ showOnlyEnergyReport = false }) => {
  ...
  const { coolingHours, heatingHours, totalFloorArea, numberOfFloors } = useFloorPlanStore();
  ...
  const setMonthlyKWhCooling = useSheetCalculationStore((state) => state.setMonthlyKWhCooling);
  const setMonthlyKWhHeating = useSheetCalculationStore((state) => state.setMonthlyKWhHeating);
  const setSheetEUI = useSheetCalculationStore((state) => state.setEUI);
  const setAppliancesEnergy = useSheetCalculationStore((state) => state.setAppliancesEnergy);
  const setLightingEnergy = useSheetCalculationStore((state) => state.setLightingEnergy);
  ...
  const totalKWhCoolingSelected = monthlyCoolingKWh.reduce((acc, val) => acc + val, 0);
  const totalKWhHeatingSelected = monthlyHeatingKWh.reduce((acc, val) => acc + val, 0);
  ...
  }, [totalKWhCoolingSelected, totalKWhHeatingSelected, setSheetSums, monthlyCoolingKWh, monthlyHeatingKWh, setMonthlyKWhCooling, setMonthlyKWhHeating]);

  // Convert floor area to square meters
  const floorAreaInM2 = Number(totalFloorArea) * 0.092903;

  // Calculate total annual energy from appliances
  ...
  const eui = (appliancesAnnualEnergy + totalKWhHeatingSelected + totalKWhCoolingSelected + lightingAnnualEnergy) / (floorAreaInM2 * Number(numberOfFloors));

  // Store EUI, appliances energy, and lighting energy in the Zustand store - CHANGE: Use lightingAnnualEnergy
  useEffect(() => {
    setSheetEUI(eui);
    setAppliancesEnergy(appliancesAnnualEnergy);
    setLightingEnergy(lightingAnnualEnergy);
  }, [eui, setSheetEUI, appliancesAnnualEnergy, lightingAnnualEnergy, setAppliancesEnergy, setLightingEnergy]);
