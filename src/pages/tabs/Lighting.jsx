...
function Lighting() {
  ...
  const [totalWattage, setLocalTotalWattage] = useState(
    storedTotalWattage || 0
  );
  const [totalAnnualEnergy, setLocalTotalAnnualEnergy] = useState(
    storedTotalAnnualEnergy || 0
  );
  ...
  const handleEditLight = (index) => {
    ...
    setCustomLightLabel(
      light.lightType.startsWith("Other") ? light.lightType : ""
    );
    setOpenLightModal(true);
  };
  ...
  const calculateAnnualEnergy = (light) => {
    ...
    return (
      (light.wattage * light.quantity * light.hoursOfUsage * days) /
      1000
    ).toFixed(2);
  };
  ...
  useEffect(() => {
    ...
    const energyTotal = lights.reduce((acc, light) => {
      if (light.annualEnergyConsumption) {
      ...
      return (
        acc +
        (light.wattage * light.quantity * light.hoursOfUsage * days) / 1000
      );
    }, 0);
    ...
    <Box p={3}>
      ...
      <Box p={2} bgcolor="#D9EAFD" borderRadius={2} textAlign="center" mb={2}>
        <Typography variant="body2" fontWeight="bold">
          Total Wattage:{" "}
          {totalWattage > 0 ? `${totalWattage.toFixed(2)} W` : "0 W"}
        </Typography>
      </Box>
      ...
      <Box p={2} bgcolor="#D9EAFD" borderRadius={2} textAlign="center">
        <Typography variant="body2" fontWeight="bold">
          ...
          {totalAnnualEnergy > 0
            ? `${totalAnnualEnergy.toFixed(2)} kWh`
            : "0 kWh"}
        </Typography>
      </Box>
