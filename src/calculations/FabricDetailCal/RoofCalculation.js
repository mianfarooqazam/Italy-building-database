...
export function calculateRoofHeatLoss(uaValue) {
  // Make sure we're working with a numeric value
  const numericUAValue = parseFloat(uaValue) || 0;
  return numericUAValue;
}

export function calculateTotalFabricHeatLoss(roofUAValue, wallUAValue, slabUAValue, windowUAValue, doorUAValue) {
  // Make sure we're working with numerical values
  const roof = parseFloat(roofUAValue) || 0;
  const wall = parseFloat(wallUAValue) || 0;
